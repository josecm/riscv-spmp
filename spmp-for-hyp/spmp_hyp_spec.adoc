== "Ssvspmp" Extension for VS-level Physical Memory Protection (vSPMP)

=== Extension Dependencies

. The `Ssvspmp` is dependent on the latest versions of Supervisor-Level ISA specification (Version 1.13), and Shbare (Version 1.0) at the time of writing.

. The `Ssvspmp` is dependent on the "Sspmp" Extension for S-level Physical Memory Protection (SPMP).
...
Indirect accesses to vSPMP CSRs are not ordered with respect to each other or with subsequent memory accesses. To enforce ordering for subsequent accesses whose effective privilege mode is VS or VU, software must execute memory memory-management fence instructions. When V = 1, a `SFENCE.VMA`instruction is required as defined in the base `Sspmp` extension. When V = 0, M or HS mode software must execute an `HFENCE.VVMA` instruction with rs1=x0 and rs2=x0, which synchronizes subsequent memory accesses with all preceding vSPMP CSR writes.

== "Ssvspmpen" Extension for Optimizing Context Switching of vSPMP Entries

=== Extension Dependencies

. The `Ssvspmpen` is dependent on `Ssvspmp`.

=== vSPMP Context Switch Optimization

The `Ssvspmpen` is an optional extension that provides a mechanism to enchance vSPMP context switches. It replicates the functionality provided by `Sspmpen` for optimizing SPMP context switches for the vSPMP.
For this, it introduces new virtual supervisor CSRs that control the activation of vSPMP entries:

* For RV64 architectures, it introduces a 64-bit WARL virtual CSR, `vspmpen`.
* For RV32 architectures, it introduces an additional 32-bit WARL CSR, `vspmpenh`, which serves as an alias for the upper 32 bits of the `vspmpen` register.

When `hstatus.VTVM` is set and when V = 1, any attempt to access `vspmpen` raises a virtual-instruction exception.

This extension is mandatory if `Sspmpen` is implemented.

== "Sshspmpdeleg" Extension for Sharing Hardware Resources between SPMP and vSPMP

=== Extension Dependencies

. The `Sshspmpdeleg` is dependent on `Ssvspmp`.

=== Resource Sharing between SPMP and vSPMP

With this extension the hypervisor can delegate entries from SPMP to vSPMP. For this, it introduces a new WARL HSXLEN-bit HS-mode CSR named `hspmpdeleg`, whose layout is detailed in <<hspmpdeleg_format_rv64>>. The `hspmpdeleg.pmpnum` field defines the number of SPMP entries reserved for hypervisor use. All SPMP entries with an index greater than or equal to `hspmpdeleg.pmpnum` are delegated to vSPMP.

[[hspmpdeleg_format_rv64]]
.hspmpdeleg CSR format.
include::images/bytefield/hspmpdeleg_format_rv64.adoc[]
...
vSPMP entries are supported contiguously and begin with the lowest vSPMP indirect CSR number.
`SPMP[hspmpdeleg.pmpnum]` through `SPMP[N-1]` map to `vSPMP[0]` through `vSPMP[N-1-hspmpdeleg.pmpnum]`, where N is the total number of SPMP entries provided by the HEE.
For instance, given 32 SPMP entries provided by the HEE and `hspmpdeleg.pmpnum` set to 16, `SPMP[16]` to `SPMP[31]` map to `vSPMP[0]` to `vSPMP[15]`.
A read of an out-of-index vSPMP entry will return 0, and a write to such a vSPMP entry will be ignored.
...
== "Smhspmpdeleg" Extension for M-mode Support of vSPMP Resource Sharing

=== Extension Dependencies

. The `Smhspmpdeleg` is dependent on `Smpmpdeleg` and `Sshspmpdeleg`.

=== Extension to `mpmpdeleg`

When `Smhspmpdeleg` is implemented, the `pmpnum` field of `mpmpdeleg` is extended from 7 bits to 8 bits. Bit 7 of `mpmpdeleg`, previously Reserved (WPRI), becomes the most significant bit of `pmpnum`. The extended layouts are shown in <<mpmpdeleg_ext_format_rv64>> and <<mpmpdeleg_ext_format_rv32>>.

[[mpmpdeleg_ext_format_rv64]]
.Extended mpmpdeleg CSR format, RV64.
include::images/bytefield/mpmpdeleg_ext_format_rv64.adoc[]

[[mpmpdeleg_ext_format_rv32]]
.Extended mpmpdeleg CSR format, RV32.
include::images/bytefield/mpmpdeleg_ext_format_rv32.adoc[]
...
=== M-mode Support for Hypervisor/Guest SPMP Resource Sharing
...
== "Sshspmpen" Extension for Optimizing Context Switching of SPMP Hypervisor/Guest Entries

=== Extension Dependencies

. The `Sshspmpen` is dependent on `Sspmp`.

=== Optimization for Context Switching of Hypervisor/Guest SPMP
...
Similarly, `spmpen` governs SPMP entries for the hypervisor's own address space, while `hspmpen` controls the guest address space enforced by SPMP. This separation allows the hypervisor to manage its own and guest memory protection configurations independently.
====
