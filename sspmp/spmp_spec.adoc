...
This CSR has two fields:
 ...
 - Bit 0. ssecfg.SMWP (Supervisor Memory Access Whitelist Policy): When set, this bit changes the default SPMP policy for S-Mode when accessing memory regions that don't have a matching rule to denied instead of ignored. This bit must reset to 0.

 - Bit 1. ssecfg.SMAL (SPMP Match-Any Logic): When set, this bit changes the entry matching logic for SPMP from the default priority-based matching, where the lowest-numbered SPMP entry that matches any byte of the access determines the permissions for that access, to a match-any logic where the final permissions for the access are the union of the permissions of any entry that matches any byte of the access. This bit must reset to 0.


For RV64 sseccfg is a 64 bits wide, while for RV32 ssecfg is divided in sseccfgl (lower 32 bits) and sseccfgh (upper 32 bits).
...
<<RV32_cfg>> and <<RV64_cfg>> demonstrate the first 16 entries of SPMP. The layout of the rest entries is similar.
...
Each SPMP address register encodes bits 33-2 of 34-bit physical address for RV32, as shown in <<spmpcfg-rv32>>.
For RV64, each SPMP address encodes bits 55-2 of a 56-bit physical address, as shown in <<spmpcfg-rv64>>.
...
=== Encoding of Permissions  [[encoding_of_permissions]]
...
2. If the effective privilege mode of the access is S and no SPMP entry matches, if ``ssecfg.SMWP`` is clear the access is ``allowed``, otherwise if ``ssecfg.SMWP`` is set, the access is denied;
...
(1) It will introduce one more layer to check permission for each memory access.
...
If page-based virtual memory is implemented, an SFENCE.VMA instruction with rs1=x0 and rs2=x0 is needed after the SPMP CSRs are written.
...
We add two CSRs called *_spmpswitch0_* and *_spmpswitch1_*, which are XLEN-bit read/write registers, as shown in <<spmpswitch-rv32>>.
For RV64, only *_spmpswitch0_* is used, as shown in <<spmpswitch-rv64>>.
...
*Direct CSR access*: SPMP CSRs can be accessed directly with corresponding CSR numbers. Direct CSR access must always be available even if the `Sscsrind` extension is implemented.
